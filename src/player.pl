...
cekInfo :-
    ...
    currColor(Y),
    format('Warna Aktif: ~w~n~n', [Y]),
    modeGame(Mode),
    (Mode == 2 ->
    playerTeam(P),
    retract(playerTeam(P)),
    playerTeam(Q),
    asserta(playerTeam(P)),
    get_index(P,0,FirstPlayer1),
    get_index(P,1,SecondPlayer1),
    get_index(Q,0,FirstPlayer2),
    get_index(Q,1,SecondPlayer2),
