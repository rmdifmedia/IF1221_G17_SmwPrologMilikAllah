:- include('player.pl').
:- include('fact.pl').
:- include('startGame.pl').
:- include('helper.pl').

inputNamaFile(NamaFileTxt) :-
    write('Masukkan nama file permainan: '),
    read(NamaFile),
    name(NamaFile, KodeASCII),
    insert_tail(KodeASCII, [46,116,120,116], KodeASCIILengkap),
    name(NamaFileTxt, KodeASCIILengkap).

insert_tail([], Tail, Tail).
insert_tail([H|T], Tail, [H|Result]) :-
    insert_tail(T, Tail, Result).

saveGame :-
    inputNamaFile(NamaFileTxt),
    open(NamaFileTxt, write, Stream),

    modeGame(Mode),
    format(Stream, 'mode_game:~w.~n', [Mode]),

    playerCount(Count),
    format(Stream, 'player_count:~w.~n', [Count]),

    listUrutan(Urutan),
    format(Stream, 'urutan_pemain: ~q.~n', [Urutan]),

    turn(Giliran),
    format(Stream, 'giliran: ~q.~n', [Giliran]),

    kartuTop(W, J),
    format(Stream, 'discard_top: ~q-~q.~n', [W, J]),

    currColor(Warna),
    format(Stream, 'warna_aktif: ~q.~n', [Warna]),

    arah(Arah),
    format(Stream, 'arah_permainan: ~q.~n', [Arah]),

    listUni(UNI),
    format(Stream, 'status_UNI: ~q.~n', [UNI]),

    listHistoryTop(History),
    format(Stream, 'list_history_top:~q.~n', [History]),

    ( Mode == 2
    -> savePlayerTeam(Urutan, Stream)
    ; true
    ),

    saveKartuTangan(Urutan, Stream),

    close(Stream),
    format('Status permainan berhasil disimpan ke ~w.~n', [NamaFileTxt]).

savePlayerTeam([], _).
savePlayerTeam([Player|Rest], Stream) :-
    playerTeamNumber(Player, TeamNum),
    format(Stream, 'player_team:~q-~w.~n', [Player, TeamNum]),
    savePlayerTeam(Rest, Stream).

saveKartuTangan([], _).
saveKartuTangan([Player|Rest], Stream) :-
    kartu_diTangan(Player, Kartu),
    format(Stream, 'kartu(~q):~q.~n', [Player, Kartu]),
    saveKartuTangan(Rest, Stream).

loadGame :-
    inputNamaFile(NamaFileTxt),
    retractall(modeGame(_)),
    retractall(playerCount(_)),
    retractall(listUrutan(_)),
    retractall(listPlayer(_)),
    retractall(turn(_)),
    retractall(kartuTop(_,_)),
    retractall(currColor(_)),
    retractall(arah(_)),
    retractall(listUni(_)),
    retractall(listHistoryTop(_)),
    retractall(playerTeamNumber(_,_)),
    retractall(kartu_diTangan(_,_)),
    open(NamaFileTxt, read, Stream),
    loadFakta(Stream),
    close(Stream),
    turn(Giliran),
    write('Status permainan berhasil diambil dari ~w.~n', [NamaFileTxt]),
    write('Melanjutkan giliran ~w...', [Giliran]).

loadFakta(NamaFile) :-
    read_term(NamaFile, Term, []),
    ( Term == end_of_file -> true 
    ; restore_term(Term),
    loadFakta(NamaFile)
    ).
    
restore_term(mode_game:Mode)         :- assertz(modeGame(Mode)).
restore_term(player_count:Count)     :- assertz(playerCount(Count)).
restore_term(urutan_pemain:Urutan)   :- assertz(listUrutan(Urutan)), assertz(listPlayer(Urutan)).
restore_term(giliran:Giliran)        :- assertz(turn(Giliran)).
restore_term(discard_top:W-J)        :- assertz(kartuTop(W, J)).
restore_term(warna_aktif:Warna)      :- assertz(currColor(Warna)).
restore_term(arah_permainan:Arah)    :- assertz(arah(Arah)).
restore_term(status_UNI:UNI)         :- assertz(listUni(UNI)).
restore_term(list_history_top:History)   :- assertz(listHistoryTop(History)).
restore_term(player_team:Player-TeamNum) :- assertz(playerTeamNumber(Player, TeamNum)).
restore_term(kartu(Player):Kartu)    :- assertz(kartu_diTangan(Player, Kartu)).
